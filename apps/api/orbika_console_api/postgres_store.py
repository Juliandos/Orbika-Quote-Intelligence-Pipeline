...
from .graph_store import load_quote_graph_context

# _database_url es una funciÃƒÂ³n que verifica si la variable de entorno DATABASE_URL estÃƒÂ¡ configurada. Si no lo estÃƒÂ¡, lanza una excepciÃƒÂ³n RuntimeError indicando que DATABASE_URL es necesario cuando ORBIKA_API_STORE estÃƒÂ¡ configurado como 'postgres'. Si DATABASE_URL estÃƒÂ¡ configurado, reemplaza el prefijo "postgresql+psycopg://" con "postgresql://" para asegurar la compatibilidad con psycopg y devuelve la URL de conexiÃƒÂ³n resultante.
def _database_url() -> str:
    if not DATABASE_URL:
        raise RuntimeError("DATABASE_URL is required when ORBIKA_API_STORE=postgres.")
    return DATABASE_URL.replace("postgresql+psycopg://", "postgresql://", 1)

# _connect es una funciÃƒÂ³n que establece una conexiÃƒÂ³n a la base de datos PostgreSQL utilizando psycopg. Utiliza la URL de conexiÃƒÂ³n proporcionada por _database_url() y configura el row_factory para que las filas devueltas por las consultas sean diccionarios en lugar de tuplas, lo que facilita el acceso a los datos por nombre de columna.
def _connect() -> psycopg.Connection:
    return psycopg.connect(_database_url(), row_factory=dict_row)

# EventBus es una clase que implementa un sistema de publicaciÃƒÂ³n-suscripciÃƒÂ³n para eventos. Permite a los productores publicar eventos y a los consumidores suscribirse para recibir esos eventos en tiempo real. La clase utiliza una cola para cada suscriptor y mantiene un historial de eventos recientes para que los nuevos suscriptores puedan recibir eventos pasados.
...
def _is_internet_match_option(item: Any) -> bool:
    if not isinstance(item, dict):
        return False
    source_type = str(item.get("source_type") or "").strip().lower()
    if source_type == "internet_search":
        return True
    match_type = str(item.get("match_type") or "").strip().lower()
    return match_type in {"web_validated", "internet_search", "web_search"}

# _quote_summary es una funciÃƒÂ³n que toma una fila de datos (row) como argumento y extrae informaciÃƒÂ³n relevante para crear un resumen de una cotizaciÃƒÂ³n. La funciÃƒÂ³n devuelve un diccionario con campos especÃƒÂ­ficos como quote_key, generated_at, received_at, subject, aviso_id, placa, marca, linea, load_status, repuestos_count, parts_with_matches, exact_reference_matches y parts_with_agentic_matches. Esta funciÃƒÂ³n se utiliza para transformar los datos crudos de la base de datos en un formato estructurado y fÃƒÂ¡cil de usar para el frontend o para otros propÃƒÂ³sitos.
def _quote_summary(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "parts_reviewed": int(row.get("parts_reviewed") or 0),
        "parts_with_agentic_matches": int(row.get("parts_with_agentic_matches") or 0),
    }

# list_quotes es una funciÃƒÂ³n que se conecta a la base de datos PostgreSQL, ejecuta una consulta SQL para obtener un resumen de las cotizaciones almacenadas en la base de datos, y devuelve una lista de diccionarios con la informaciÃƒÂ³n resumida de cada cotizaciÃƒÂ³n. La funciÃƒÂ³n utiliza _quote_summary para transformar cada fila de resultados en un formato estructurado y fÃƒÂ¡cil de usar.
    ...
    with _connect() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            SELECT
              ...
              COUNT(DISTINCT ar.part_id)::int AS parts_reviewed,
              COUNT(DISTINCT ar.part_id) FILTER (WHERE COALESCE(jsonb_array_length(ar.selected_options), 0) > 0)::int
                AS parts_with_agentic_matches
            FROM quotes q
...
# get_quote_detail es una funciÃƒÂ³n que se conecta a la base de datos PostgreSQL, ejecuta una consulta SQL para obtener los detalles de una cotizaciÃƒÂ³n especÃƒÂ­fica identificada por quote_key, y devuelve un diccionario con toda la informaciÃƒÂ³n detallada de esa cotizaciÃƒÂ³n. La funciÃƒÂ³n tambiÃƒÂ©n obtiene informaciÃƒÂ³n relacionada como partes, coincidencias con proveedores y revisiones agentic, y organiza todos estos datos en un formato estructurado para su uso en el frontend o para otros propÃƒÂ³sitos. Si no se encuentra la cotizaciÃƒÂ³n, devuelve None.
...
def get_quote_detail(quote_key: str) -> dict[str, Any] | None:
    ...
    for part in parts:
        ...
        if not isinstance(selected_options, list):
            selected_options = []
        internet_matches = [item for item in selected_options if _is_internet_match_option(item)]
        agentic_matches = [item for item in selected_options if not _is_internet_match_option(item)]
        agentic_parts.append(
            {
                "part_name": part["name"],
                "top_provider_id": agentic_matches[0].get("provider_id") if agentic_matches else None,
                ...
                "selected_matches": agentic_matches,
                "internet_matches": internet_matches,
                "internet_search": {
                    "parts_with_internet_matches": 1 if internet_matches else 0,
                    "validated_links": len(internet_matches),
                },
            }
        )
    ...
    payload = {
        ...
        "agentic_supplier_matching": {
            "summary": {
                "parts_total": parts_total,
                "parts_reviewed": parts_total,
                "parts_with_agentic_matches": parts_with_agentic_matches,
            },
            "parts": agentic_parts,
        },
    }
    payload["graph_context"] = load_quote_graph_context(str(quote["quote_key"]))
    return _jsonable(payload)

# _match_payload es una funciÃƒÂ³n que toma un diccionario de coincidencia (match) como argumento y extrae informaciÃƒÂ³n relevante para crear un resumen de esa coincidencia. La funciÃƒÂ³n devuelve un diccionario con campos especÃƒÂ­ficos como provider_id, provider_name, product_name, reference, sku, brand, category_name, subcategory_name, detail_url, price, currency, availability, match_type, score_percent, rank, reasons y risk_flags. Esta funciÃƒÂ³n se utiliza para transformar los datos crudos de la base de datos en un formato estructurado y fÃƒÂ¡cil de usar para el frontend o para otros propÃƒÂ³sitos.
