...
- Autorecambios LTDA required treating the catalog as a single listing surface with HTML pagination on the same URL, so the extractor had to track page state instead of URL changes and wait for the grid to actually change before advancing.
- The working route was to inspect the rendered page, discover the Divi Plus AJAX endpoint behind the product grid, and crawl `https://autorecambiosltda.com/repuestos-para-carros-y-camionetas/` page by page instead of following categories or secondary surfaces.
- The site can hide catalog entries behind stock-state filtering, so the listing crawl needed to preserve the visible grid first and then probe the rendered props when the goal was catalog completeness.
- A premature page turn can skip the last card of a row or page, so the extractor must finish reading the current grid, capture every visible product, and only then move forward.
